def global_mean(train, test):
    """
        Define the global mean model for recommendation.

        Input:
            train (Pandas Dataframe) : train dataset
            test (Pandas Dataframe): test dataset

        Output:
            (Pandas Dataframe): test dataset with updated predictions calculated with global mean

    """

    output = test.copy()

    # calculate the global mean
    global_mean_train = train.Prediction.mean()

    output.Prediction = global_mean_train

    def round_pred(row):
        return round(row.Prediction)

    output['Prediction'] = output.apply(round_pred, axis=1)
    return output


def user_mean(train, test):
    """
        Define the user mean model for recommendation. recommends based on user's average rating.

        Input:
            train (Pandas Dataframe) : train dataset
            test (Pandas Dataframe): test dataset

        Output:
            (Pandas Dataframe): test dataset with updated predictions calculated with user mean

    """

    output = test.copy()
    train.User = train.User.astype(int)
    train.Prediction = train.Prediction.astype(int)

    # calculate the mean for each user
    mean_pred_by_user = train.groupby('User')[['Prediction']].mean()

    def assign_mean(row):
        return mean_pred_by_user[mean_pred_by_user.index == row.User].iloc[0].Prediction

    def round_pred(row):
        return round(row.Prediction)

    output['Prediction'] = output.apply(assign_mean, axis=1)
    output['Prediction'] = output.apply(round_pred, axis=1)
    return output


def movie_mean(train, test):
    """
        Define the movie mean model for recommendation. recommends based on movie's average rating.

        Input:
            train (Pandas Dataframe) : train dataset
            test (Pandas Dataframe): test dataset

        Output:
            (Pandas Dataframe): test dataset with updated predictions calculated with movie mean

    """

    output = test.copy()
    train.Movie = train.Movie.astype(int)
    train.Prediction = train.Prediction.astype(int)

    # calculate the mean for each movie
    mean_pred_by_movie = train.groupby('Movie')[['Prediction']].mean()

    def assign_mean(row):
        return mean_pred_by_movie[mean_pred_by_movie.index == row.Movie].iloc[0].Prediction

    def round_pred(row):
        return round(row.Prediction)

    output['Prediction'] = output.apply(assign_mean, axis=1)
    output['Prediction'] = output.apply(round_pred, axis=1)
    return output


def global_median(train, test):
    """
    Define the global median model for recommendation.

    Args:
        train (Pandas Dataframe) : train dataset
        test (Pandas Dataframe): test dataset

    Returns:
        output (Pandas Dataframe): test dataset with updated predictions calculated with global mean
    """

    output = test.copy()  # calculate the global median
    global_median_train = train.Prediction.median()

    output.Prediction = global_median_train

    def round_pred(row):
        return round(row.Prediction)

    output['Prediction'] = output.apply(round_pred, axis=1)
    output['Rating'] = output['Prediction']

    return output


def user_median(train, test):
    """
        Define the user median model for recommendation. recommends based on median of user's ratings.

        Input:
            train (Pandas Dataframe) : train dataset
            test (Pandas Dataframe): test dataset

        Output:
            (Pandas Dataframe): test dataset with updated predictions calculated with median mean

    """

    output = test.copy()
    train.User = train.User.astype(int)
    train.Prediction = train.Prediction.astype(int)

    # calculate the median for each user
    median_pred_by_user = train.groupby('User')[['Prediction']].median()

    def assign_median(row):
        return median_pred_by_user[median_pred_by_user.index == row.User].iloc[0].Prediction

    def round_pred(row):
        return round(row.Prediction)

    output['Prediction'] = output.apply(assign_median, axis=1)
    output['Prediction'] = output.apply(round_pred, axis=1)
    return output


def movie_median(train, test):
    """
        Define the movie median model for recommendation. recommends based on median of movie's ratings.

        Input:
            train (Pandas Dataframe) : train dataset
            test (Pandas Dataframe): test dataset

        Output:
            (Pandas Dataframe): test dataset with updated predictions calculated with movie mean

    """

    output = test.copy()
    train.Movie = train.Movie.astype(int)
    train.Prediction = train.Prediction.astype(int)

    # calculate the mean for each movie
    median_pred_by_movie = train.groupby('Movie')[['Prediction']].median()

    def assign_median(row):
        return median_pred_by_movie[median_pred_by_movie.index == row.Movie].iloc[0].Prediction

    def round_pred(row):
        return round(row.Prediction)

    output['Prediction'] = output.apply(assign_median, axis=1)
    output['Prediction'] = output.apply(round_pred, axis=1)
    return output
